        ...
        for product in group_deal_dict:
            ...
            # loop through the dict removing the items until total_quantity is 0
                ...
                if group_deal_dict[product] < item_count:
                    if (item_count - group_deal_dict[product]) >= remainder:
                ...
                # if group_deal_dict[product] > 0:
                #     item_count -= 1
                #     group_deal_dict[product] -= 1
                #     break
                # else:
                #     group_deal_dict[product] = remainder
...
def group_deal_remainder_cost_calculation(group_deal_dict: dict) -> int:
    ...
    s_total = group_deal_dict.get("S") * 20
    t_total = group_deal_dict.get("T") * 20
    x_total = group_deal_dict.get("X") * 17
    y_total = group_deal_dict.get("Y") * 20
    z_total = group_deal_dict.get("Z") * 21
